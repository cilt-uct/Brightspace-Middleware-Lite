...
class Course(object):
    ...
    def get_course_by_id(self, org_unit_id: int, params: dict = None) -> Response:
        ...
        """
        return self._client._do_get("{}courses/{}".format(self._client.lp_url, org_unit_id), params=params)
    ...
    def get_course_by_code(self, org_code: str, name: str = None, first: bool = False) -> Response:
        ...
        Returns:
            Response: This action returns a CourseOffering JSON block with the provided course’s information.
        """
        url = f'{self._client.lp_url}orgstructure/?exactOrgUnitCode={urllib.parse.quote(org_code)}&orgUnitType={COURSE_OFFERING_TYPE}'
        if name:
            url = f'{url}&exactOrgUnitName={name}'
    ...
    def get_course(self, val: typing.Union[int, str], use_id: bool = False, params: dict = None) -> Response:
    ...
    def create_course_template(self, name: str, code: str, path: str, parent_org_id: int) -> Response:
        ...
        return self._client._post("{}coursetemplates/".format(self._client.lp_url), data=payload)

    def delete_course_template(self, org_unit_id: str):
        return self._client._delete("{}coursetemplates/{}".format(self._client.lp_url, org_unit_id))
    ...
    def create_site_direct(self, name: str, code: str, path: str, template_id: int, semester_id: int) -> Response:
        ...
        # print(payload)
        return self._client._post("{}courses/".format(self._client.lp_url), data=payload)

    def create_site(self, course_code: str, name: str, template_id:int, semester_id:int,
                    create_even_if_it_exists: bool = False, check_name: bool = False, active: bool = False):
        ...
        if create_even_if_it_exists or _found_course['status'] != "success":
            ...
            if new_course['status'] == 'success':
                ...
                if update_path['status'] == 'success':
                    new_course['data']['Path'] = '/content/enforced/{}/'.format(new_course['data']['Identifier'])
    ...
    def create_community_site(self, name: str, faculty: str, term: int = 0, course_code:str = '',
                              create_even_if_it_exists: bool = False, check_name: bool = False, active: bool = True):
        # print(locals())
        ...
        semester_id = Utils.get_id_from_valid_or_default(self._client.orgunit.get_semester_by_term, site_term, datetime.now().year)
        template_id = Utils.get_id_from_valid_or_default(self.get_course_template, f'{faculty}_other_template', 'UCT_other_template')
        ...
        if semester_id > 0:
            if template_id > 0:
                ...
                return self.create_site(course_code=course_code,
                                            name=name,
                                            template_id=template_id, semester_id=semester_id,
    ...
    def create_course_site(self, course_code: str = '', name: str = '', template: str = '', term: int = 0, dept:str = 'other',
                           create_even_if_it_exists: bool = False, check_name: bool = False, active: bool = True):
        # print(locals())
        ...
        department_id = Utils.get_id_from_valid_or_default(self._client.orgunit.get_department_by_code, dept, 'other')
        semester_id = Utils.get_id_from_valid_or_default(self._client.orgunit.get_semester_by_term, site_term, datetime.now().year)
    ...
    def delete_course(self, org_unit_id: str):
        return self._client._delete("{}courses/{}".format(self._client.lp_url, org_unit_id))
    ...
    def get_course_classlist_by_id(self, org_unit_id: int, filter_role:int = 0, paged: bool = False, bookmark: str = None) -> Response:
        ...
        Warning:
            Some orgunits can have very large numbers of enrolled users, so the classlist can be a quite large collection.
            We advise you to make very careful use of this API call because it asks for an unbounded set of results.
            We strongly recommend you use the paged version of this call instead.

        """
        # NOTE: heed the warning ...
        if not paged:
            return self.filter_by_role(self._client._do_get("{}{}/classlist/".format(self._client.le_url, org_unit_id)), filter_role)

        if bookmark:
            return self.filter_by_role(self._client._do_get("{}{}/classlist/paged/?bookmark={}".format(self._client.le_url, org_unit_id, bookmark)), filter_role)

        return self.filter_by_role(self._client._do_get("{}{}/classlist/paged/".format(self._client.le_url, org_unit_id)), filter_role)

    def get_course_enrollments_by_id_by_page(self, org_unit_id:int, bookmark:str = None, roleId:int = 0, active:int = -1) -> Response:
        return self.get_course_enrollments_by_id(org_unit_id = org_unit_id, bookmark = bookmark, roleId = roleId, active=active)

    def get_course_enrollments_by_id(self, org_unit_id:int, bookmark: str = None, roleId:int = 0, active:int = -1) -> Response:
        ...
        Returns:
            Response: This action returns a paged result set containing the resulting OrgUnitUser data blocks for the segment
        ...
        # print(f'{params=}')
        return self._client._do_get("{}/enrollments/orgUnits/{}/users/".format(self._client.lp_url, org_unit_id), params = params)
    ...
    def get_enrollment_by_id_for_user_id(self, org_id:int, user_id:int):
        ...
        return self._client._do_get("{}enrollments/orgUnits/{}/users/{}".format(self._client.lp_url, org_id, user_id))
    ...
    def enroll_user_in_role(self, org_unit_id: int, user_id: int, role_id: int) -> Response:
        ...
        return self._client._post("{}enrollments/".format(self._client.lp_url), data=payload)
    ...
    def enroll_any_user(self, org_id: int = 0, user_id:int = 0, eid:str = None, role:int = 0, add_to_announcement:bool = True) -> Response:
        ...
        if int(user_id) > 0 or eid:
            ...
            if _found_user['status'] == 'success':
                if _found_user['data']['Activation']['IsActive'] == True:
                    ...
                    if result['status'] == 'success' and role in TEACHING_ROLES:
                        ...
                        if add_to_announcement and ama_announcement_site['status'] == 'success':
                            self.enroll_user_in_role(ama_announcement_site['data']['Identifier'], _found_user['data']['UserId'], 121) # Member

                    return result
                else:
                    return self._client.return_error_state(f"User {_found_user['data']['UserName']} ({_found_user['data']['UserId']}) is Inactive")
        ...
        if eid:
            ...
            if _found_user['status'] == 'success':
                if _found_user['data']['Activation']['IsActive'] == True:
                    ...
                    return self._client.return_success_state(f"Student enrolled in all sites")
                else:
                    return self._client.return_error_state(f"User {_found_user['data']['UserName']} ({_found_user['data']['UserId']}) is Inactive")
    ...
    def import_file(self, org_unit_id: int, filename: str, params: dict = None) -> Response:
        ...
        url = "{}import/{}/imports/".format(self._client.le_url, org_unit_id)
        payload={}
    ...
    def copy_course_content(self, target_org_unit:int, src_org_unit:int, components = None) -> Response:
        ...
        return self._client._post("{}import/{}/copy/".format(self._client.le_url, target_org_unit), data=payload)

    def get_users_by_role(self, org_unit_id:int, role:str=None, onlyActive:bool = True) -> list:
