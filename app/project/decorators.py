...
from .db.models import ServiceNowLog
from project.app import alchemy_db

SERVICE_NOW_USERNAME = "servicenow_script"

def process_service_now_request(func):
    @wraps(func)
    def decorated_function(*args, **kwargs):
        should_log = False
        user_identifier = None

        # Case 1: Authenticated via Flask-Login
        if current_user.is_authenticated:
            if current_user.eid == SERVICE_NOW_USERNAME:
                should_log = True

        # Case 2: Not authenticated yet, but request uses Basic Auth
        elif request.authorization:
            if request.authorization.username == SERVICE_NOW_USERNAME:
                should_log = True

        # We're not intrested in this request as it's not from ServiceNow
        if not should_log:
        ...
        try:
            ...
            # log error case
            alchemy_db.session.add(ServiceNowLog(
        ...
        alchemy_db.session.add(ServiceNowLog(request=json.dumps(request_body),
...
def is_service_now_request():
    return current_user.eid == "servicenow_script"
